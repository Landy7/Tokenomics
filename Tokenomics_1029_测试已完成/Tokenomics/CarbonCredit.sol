...
contract CarbonCredit{
    ...
    //返回第三方认证的编码
    function getIDByThirdParty()view external returns(uint256[] memory){
        return IDByThirdParty;
    }

    //设置第三方认证的唯一编码的carboncredit
    function setIDByThirdParty(uint256[] memory _IDByThirdParty) external {
        IDByThirdParty = _IDByThirdParty;
    }
    ...
    function setAVALIABLE_TIME(uint256 _AVALIABLE_TIME, uint256 _Uint) external {
        if(_Uint == 0){
            ...
            AVALIABLE_TIME = _AVALIABLE_TIME * 30 days;
        }else if(_Uint == 5){
