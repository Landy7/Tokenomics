...
contract IncentivePolicyMVPV3{
   ...
   //只能运行16个 variables
   //1028: 之后这里删掉，把variables存储在User合约里面
    struct Voter{
    ...
    //获取用户的Type Of Carbon Coin
    function getTypeOfCarbonCoinForUser() view external returns (CarbonCoinV2[] memory){
        CarbonCoinV2[] memory cc_v2 = voters[msg.sender].carboncoin;
        return cc_v2;
    }
    ...
    // //测试用的，之后会删除
    // function TestExpectedDataByInput(Travel_mode memory _expect_mode) external {
    ...
    function calculateCurrentCarbonCost() external{
        ...
        sender.allCarbonSaving += sender.currentWeek_carbonSaving; //用户总共节省了多少碳
    ...
    //每到每个星期五的23:59:59 来进行统计所有用户这周节省了多少碳 -----该函数所有用户都只执行一次，
    //并且必须为chairpersion才能执行，之后设计成guildMember来执行
    //超出了原本碳计划的用户就不计算在内
    //需要倒计时功能 ---- 在前端去添加该功能，然后调用该函数
    function calculateEachUserCarbonSavings() external returns(int){
        //还没有初始化
        require(voters[msg.sender].target_carbon_allowance != 0,"you have not initialize!");
        require(msg.sender == chairperson, "only chairperson can use this function"); //之后需要换成guild member
        require(voters[msg.sender].isVoted != 0, "you cannot operate this function");
        
        int currentWeekCarbonSavings = 0;
        for(uint i = 0; i < allUserAddress.length; i++){
            //判断用户是否节省了碳
            if(voters[allUserAddress[i]].currentWeek_carbonSaving > 0){
        ...
        verifyAllUserCarbonSaving.push(currentWeekCarbonSavings); //该round节省了多少碳
        ...
        round++;
        return currentWeekCarbonSavings; //获取当前周节省了多少碳,以及当前周是多少
    }


    //发送给guild member
    event verify(uint roundToVerifyThirdParty, int _carbonSavingValue, address sender);

    //直到多少round, carbon值已经达标，可以提交给第三方 提醒guild member
    function verifyToThirdParty(uint _roundToVerifyThirdParty, int carbonSaving)internal{
        //address硬编码，需要修改
    ...
    function mintCarbonCoinForSpecificRound(CarbonCredit carbonCredit) external returns(CarbonCoinV2){
        require(msg.sender == chairperson, "only chairperson can use this function");
        require(!verifiedCarbonCredit[carbonCredit], "this carbon credit is used!");
        int carboncoinAmount = 0;
        //需要先验证
        if(carbonCredit.verify(carbonCredit)){
            carboncoinAmount = int(carbonCredit.getNumber()) * 1000; //新mint出来的carboncoin
        }
        ...
        // cc_1.setCarbonCoinRetireTimeStamp(carbonCredit.getCarbonCreditRetireTimeStamp()); //设置过期时间
    ...
    function distributeToEachUsers(uint thisRound) external{
        ...
        //检查上一次验证的轮数是多少？
        while(lastVerifiedRound > 0 && getCarbonCoinAmountForSpecificRound[lastVerifiedRound-1] == 0){
            lastVerifiedRound--;
        }
        
        //记录每一轮直到交给第三方验证的轮数，这期间每个用户省下了多少carbon
        for(uint256 i = lastVerifiedRound; i<= thisRound;i++){
        ...
        for(uint i = 0; i < EachCarbonSavingUserForVerifiedSpecificRound.length; i++){
            ...
            verifyUniqueCarbonSavingUser[EachCarbonSavingUserForVerifiedSpecificRound[i]] = false; //恢复默认false
        }

        getVerifiedRound[thisRound] = true; //该round已经被验证过
        delete EachCarbonSavingUserForVerifiedSpecificRound; //清空
    }
    ...
    //查看是否过期
    //问题：价格怎么去定？
    //functiin存在原子性，require会直接让整个function终止，导致执行了一部分的function代码也会失效
    //要么全部执行，要么全部不执行
    //1028: 之后该部分代码用新的smart contract 重构
    function transferToCarbonCredit(uint256 value, CarbonCredit _carboncredit) external returns(bool){
        require(!_carboncredit.getIsBurn(),"this Carbon Credit is burned!"); //查看该carboncredit是否过期
        //判断当前carbon credit是否过期？过期就直接burn掉
        if(burnTheRetringCarbonCoin(_carboncredit)){
            return false;
        }

        //msg.sender 花费 value个token 从 carbonCreditOwner 手里买 carbon credit
        if(purchaseTheCarbonCreditAndThirdParty(value)){
            _carboncredit.setCarbonCreditOwner(msg.sender); //owner转换
        }else{
            return false; //返回错误
        }
        return true;
    ...
    //用户买carboncredit & 第三方的东西（设置 public）
    function purchaseTheCarbonCreditAndThirdParty(uint256 value) public returns (bool){
        if(voters[msg.sender].AllCarbonCoin < value) return false;
        voters[msg.sender].AllCarbonCoin -= value;
        CarbonCoinV2[] storage ccv2 = voters[msg.sender].carboncoin;
        uint i = 0;
        while(i < ccv2.length && value > 0){
            uint256 amount = voters[msg.sender].TheNumberOfEachTypeCarbonCoin[ccv2[i]]; //减去最优先过期的carboncoin
            //该carboncoin还有剩
            if(amount > value){
                amount -= value;
            //该carboncoin没有剩
    ...
    //找到carboncredit对应过期的carboncoin, 并进行删除
    function burnTheRetringCarbonCoin(CarbonCredit carboncredit) public returns (bool) {
        if(block.timestamp < carboncredit.getCarbonCreditRetireTimeStamp()){
            return false; //还未过期
        }
        ...
        for(uint i = 0; i < allUserAddress.length; i++){
            ...
            for(uint j = 0; j < ccv2.length; j++){
                if(ccv2[j].getCarbonCreditID() == carboncredit.getCarbonCreditID()){
                    //获得每类carboncoin的数量
    ...
    //测试数据：
    //user1: r1: [123,4,1,23,5], [65,5,5,10,3]
    //user2: r1: [45,21,23,112,23], [23,12,12,50,2]
    //user3: r1: [100,0,2,59,2], [51,0,2,25,12]

    //user1: r2: [100,23,2,11,20], [50,5,2,3,6]
    //user2: r2: [40,30,32,60,22], [20,10,10,50,4]
    //user3: r2: [98,2,2,60,2], [60,0,2,10,4]

    //user1: r3: [99,21,4,20,25], [75,7,5,5,4]
    //user2: r3: [43,20,33,59,23], [20,10,15,40,4]
    //user3: r3: [80,0,20,40,4], [56,0,15,15,4]

    //user1: r4: [100,23,2,11,20], [78,5,2,3,6]
    //user2: r4: [40,30,32,60,22], [20,10,10,50,4]
    //user3: r4: [98,2,2,60,2], [80,0,2,10,4]
